...
def table_read(table):
    ...
    for line in lines:
        ...
        for position in range(len(weight)):
            weight[position] = int(weight[position])
        table.append({'weight': weight, 'name': item})
    return table

json_path = "tables/loot_tables/magic_items/tableI.json"
# Writing to sample.json
with open(json_path, "w") as outfile:
    outfile.write(json.dumps(table_read(table), indent=4))
